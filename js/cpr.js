...
function Sun() {
	...
	var renderScene = new function renderScene(){
        ...
        SunLocation(sphere, Hours, Minutes);
...
function SunLocation(sun, Hours, Minutes){
    //태양 위치 구하는 공식
    ...
    //태양 위치를 변화
    sun.position.x = -x; //-X인 이유는 왼쪽에서 오른쪽 기준인데 현실은 오른쪽에서 왼쪽으로 움직이기 때문에 -를 붙임
    sun.position.y =  y;
}
